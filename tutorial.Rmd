...
A few examples from a web site that aggregated (my) google location history are here:
...
# Real-world problems, unsolved

## Practical

* fitting GPS fixes to a road network (map matching); initial work can be found at https://github.com/edzer/fuzzyMM
...
* full support for non-points (lines, polygons, grids) of movement data (e.g. oil spills, fires)
...
* represent spatial data in an easier way (list column in data.frame), [simple features for R](http://github.com/edzer/sfr/) (see my Wednesday talk)
...
* We need to think in underlying phenomena (composed of $S, T, Q, D$):
...
* Read more in our paper on [Modelling spatiotemporal information generation](http://pebesma.staff.ifgi.de/generativealgebra.pdf)
